...
async function getCanalVendasPrivado(interaction) {
    const canalId = dbc.get(`canais.vendas_privado`)
    if (!canalId) {
        console.error("[GerarPix] Canal de vendas privado não configurado (canais.vendas_privado).")
        return null
    }
    const channel = interaction.guild.channels.cache.get(canalId)
    if (!channel) {
        console.error(`[GerarPix] Canal de vendas privado "${canalId}" não encontrado ou bot sem acesso.`)
        return null
    }
    return channel
}
...
async function sendMessagePixGerado(interaction, valor) {
    try {
        ...
        const channel = await getCanalVendasPrivado(interaction)
        if (!channel) return
        await channel.send({ embeds: [embed]})
    } catch (error) {
        console.error("[GerarPix:sendMessagePixGerado] Erro:", error)
    }
}
...
async function sendMessagePixCancelado(interaction, valor) {
    try {
        ...
        console.error("[GerarPix:sendMessagePixCancelado] Erro:", error)
    }
}
...
async function sendMessagePixBlocked(interaction, valor, banco) {
    try {
        ...
        console.error("[GerarPix:sendMessagePixBlocked] Erro:", error)
    }
}
...
async function sendMessagePixSucesso(interaction, valor, doc) {
    try {
        ...
        if (!doc?.data?.id) {
            console.error("[GerarPix:sendMessagePixSucesso] doc.data.id ausente, botão de reembolso não pode ser criado com id válido.")
        }
        ...
        try {
            ...
            console.error("[GerarPix:sendMessagePixSucesso] Erro ao gravar estatísticas de venda:", dbError)
        }
        ...
        console.error("[GerarPix:sendMessagePixSucesso] Erro:", error)
    }
}
...
async function sendMessagePixExpirado(interaction, valor) {
    try {
        ...
        console.error("[GerarPix:sendMessagePixExpirado] Erro:", error)
    }
}
